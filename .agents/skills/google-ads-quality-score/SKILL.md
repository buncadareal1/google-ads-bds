...
> ⚠️ **ĐÍNH CHÍNH BẮT BUỘC ĐỌC (QA đối chiếu doc Google chính thức, 2026-07-28 — chi tiết + nguồn: `research/google-official-curriculum.md` §F2).** Skill này có 6 điểm SAI so với support.google.com/google-ads/answer/6167118 — dùng phần CHẨN ĐOÁN THEO THÀNH PHẦN (Expected CTR / Ad Relevance / LP Experience), BỎ QUA các claim sau:
> 1. Công thức `CPC = AdRank_dưới / QS` — SAI. Google: "Quality Score is **not an input in the ad auction**" (auction dùng tín hiệu real-time, QS chỉ là chỉ số chẩn đoán 1-10).
> 2. Bảng "QS → CPC multiplier" — không có nguồn, suy từ công thức sai. Đừng trích cho khách.
> 3. "QS mặc định = 6 cho keyword mới" — SAI. Ô "—" nghĩa là chưa đủ search khớp exact, không phải điểm 6.
> 4. Trọng số thành phần 55/22/22 — Google không publish, đừng trình bày như fact.
> 5. "Google removed historical QS columns in 2022" — SAI, cột `(hist.)` vẫn tồn tại.
> 6. "Track weighted average QS cấp account" — Google cấm tường minh: QS "should not be optimized **or aggregated**".
