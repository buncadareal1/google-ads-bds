...
| 17 | **`protobuf_helpers.field_mask()` BỎ QUA field vừa set = `False`/0/rỗng** (proto3 coi là giá trị mặc định). Dính thật 15/08: tắt Display → mutate trả về OK, read-back vẫn `display=True` | Khi set field về `False`/0: tự thêm path — `op.update_mask.paths.append('network_settings.target_content_network')`. `ghi()` bắt được vì có read-back bắt buộc; không có nó thì đã tưởng là xong |
| 18 | **Search campaign mặc định BẬT Display Expansion** (`network_settings.target_content_network=True`) — nằm im nhiều ngày rồi bùng khi nâng bid. Dính thật 14-15/08: Display ăn 1,03tr / 54 click / **0 conv**, ~50% ngân sách, impr 5-6h sáng | Dựng campaign Search: set `target_content_network=False` NGAY. Kiểm định kỳ bằng `segments.ad_network_type` (SEARCH vs CONTENT) — search terms không giải thích nổi impression là dấu hiệu đầu tiên |
